...
import {
  ...
  getLoggedCacheReadTokens,
  getLoggedCacheCreationTokens,
...
export async function handleChatCore({
  ...
  if (!stream) {
    ...
    if (usage && typeof usage === "object") {
      const inputTokens = getLoggedInputTokens(usage);
      const outputTokens = getLoggedOutputTokens(usage);
      const cacheReadTokens = getLoggedCacheReadTokens(usage);
      const cacheCreationTokensLog = getLoggedCacheCreationTokens(usage);
      const totalTokens = inputTokens + outputTokens;

      const msg = `[${new Date().toLocaleTimeString("en-US", { hour12: false, hour: "2-digit", minute: "2-digit" })}] 📊 [USAGE] ${provider.toUpperCase()} | in=${inputTokens} | cache_read=${cacheReadTokens} | cache_write=${cacheCreationTokensLog} | out=${outputTokens} | total=${totalTokens}${connectionId ? ` | account=${connectionId.slice(0, 8)}...` : ""}`;
    ...
    if (translatedResponse?.usage) {
      ...
      const normalized = normalizeUsageForClient(buffered);
      translatedResponse.usage = filterUsageForFormat(normalized, sourceFormat);
    } else {
